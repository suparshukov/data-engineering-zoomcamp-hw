...
def test_vendor_id(output, *args) -> None:
    assert output['vendor_id'].isin({1,2}).sum() == len(output)


@test
def test_passenger_count(output, *args) -> None:
    assert (output['passenger_count'] > 0).sum() == len(output), \
        "There are rides with zero passengers"


@test
def test_trip_distance(output, *args) -> None:
    assert (output['trip_distance'] > 0).sum() == len(output), \
        "There are rides with zero trip distace"
